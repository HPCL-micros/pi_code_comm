...
class NeighborHandle
{
    ...
    NeighborHandle(int r_id)
    {
        ...
        //0708subStatus = n.subscribe(ss.str(), 1000, &NeighborHandle::statuscb,this);
        /*
        ss<<"/uav"<<r_id<<"/fix";
...
//0708static list<NeighborHandle*> neighbor_list;
vector<NeighborHandle*> neighbor_list;
...
{
    for(vector<NeighborHandle*>::iterator i=neighbor_list.begin();i!=neighbor_list.end();i++)
    {
        if (r_id == (*i)->_r_id)
            return true;
    }
...
void rcv_thread()
{
   ...
   int count = 0;
    while(ros::ok())//ToDo
    {
            string rcv_s;//rcvstring
            int id=0;double px=0,py=0,vx=0,vy=0;//deserialize            
...
public:
   ...
   //msg.linear.y=(rand()%100)/100.0;
    ...
    {
        int neighborListSize=1000;
        for(int i=0;i<neighborListSize;i++)
        {
            NeighborHandle* x= new NeighborHandle(i);
            neighbor_list.push_back(x);
        }
        cout<<"[flocking]NeighborListSize:"<<neighborListSize<<endl;
